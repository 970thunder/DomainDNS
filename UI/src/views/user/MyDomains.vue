				...
				<tbody>
					<tr v-for="domain in domains" :key="domain.id">
						...
						<td data-label="操作" class="row">
							<button class="btn outline" @click="editRemark(domain)" :disabled="isDeleting">
								编辑备注
							</button>
		...
		<div class="modal" v-if="showEditModal" @click="closeEditModal">
			<div class="modal-content" @click.stop>
				...
				<div class="modal-actions">
					...
					<button class="btn primary" @click="saveRemark" :disabled="isSaving">
						{{ isSaving ? '保存中...' : '保存' }}
					</button>
				</div>
			</div>
...
const editRemark = (domain) => {
	...
	editRemarkText.value = domain.remark || ''
...
const saveRemark = async () => {
	...
	try {
		...
		if (response.code === 0) {
			...
			if (domain) {
				domain.remark = editRemarkText.value
			}
			closeEditModal()
		} else {
